def fix_verifier_properly():
    # Let's read the file
    with open(r"d:\Projects\DemoSearch\fags\verifier.py", "r", encoding="utf-8") as f:
        lines = f.readlines()
        
    # We will filter out any duplicated imports and classes
    # The original verifier.py has 256 lines (we viewed it).
    # Since it was modified, it might be longer now.
    # Let's keep only the lines up to the start of "EmbeddingVerifier" to revert the file.
    reverted_lines = []
    for line in lines:
        if "EmbeddingVerifier" in line:
            break
        reverted_lines.append(line)
        
    content = "".join(reverted_lines)
    
    # Remove RELATION_DESCRIPTIONS block if it was partially inserted or already exists in some weird place
    # We will perform a clean search and replacement.
...
"""
    ...
    # We will insert RELATION_DESCRIPTIONS right before the Verifier class definition.
    content = content.replace("class Verifier:", desc_code + "\nclass Verifier:")
    
    # Now append the EmbeddingVerifier at the end
...
"""
    ...
    print("Fixed verifier.py properly.")

if __name__ == "__main__":
    fix_verifier_properly()
