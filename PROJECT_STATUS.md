...
**Reading:** the hypothesis about *why* best-first failed is confirmed — adding explicit exploration pressure via UCB1 completely fixes the tunnel-vision collapse (MCTS beats best-first by 10-15x on Large at low budgets, instead of being stuck near 0%). But it still falls short of beam search at every budget tested. At these small node budgets (5-40 nodes), MCTS's tree overhead — most simulations spend budget on selection/expansion bookkeeping near the root rather than pushing rollouts deep — apparently can't out-produce beam search's simple guarantee of K genuinely distinct full-depth hypotheses every hop. MCTS typically needs thousands of simulations to pay off; here it gets a budget of single-digit-to-low-double-digit *nodes*, which may simply be the wrong regime for it. Net: a real, partial validation of the exploration-vs-exploitation theory, but not a win — beam search's fixed-width guarantee remains undefeated by every paradigm and rule-level variant tried (six in total: §12-15 plus this one).

## 17. Net takeaway (updated)
...
- **Six independent refinement/replacement attempts on top of or instead of plain beam search all failed to beat it**: hard diversity-capping, FPG composition, sum aggregation, and soft diversity penalty (§12-14, rule-level tweaks); global best-first search (§15, catastrophic paradigm replacement); and MCTS (§16, a much better-behaved paradigm replacement that still falls short). Plain beam search's fixed-width design is not an arbitrary choice — it's been stress-tested against everything from small rule tweaks to a fundamentally different tree-search algorithm and has not been beaten once.
- **Revised recommendation:** stop iterating on FAGS's commit-then-recover design — seven independent experiments now confirm it's not salvageable. Plain (uncapped, mean-aggregation, fixed-width) beam search remains the production recommendation. If going further: the main open angle left is improving the verifier signal itself (the gold-rank ceiling from §4 was never re-measured against beam search specifically) — search-algorithm space has now been explored fairly broadly (rule tweaks, global best-first, MCTS) without finding anything better.

## 18. State of the repo / housekeeping notes
