...
- **Five independent refinement attempts on top of plain beam search all failed to beat it**: hard diversity-capping (§12), FPG composition (§13), sum aggregation (§14), soft diversity penalty (§14), and — most decisively — switching to a global best-first pruning paradigm instead of fixed-width beam (§15), which lost all 12 configurations tested with extreme significance. Plain beam search's specific design (fixed K alive at every hop, guaranteeing breadth regardless of how good any one hypothesis looks) is not an arbitrary choice — §15 shows it's load-bearing.
- **Revised recommendation:** stop iterating on FAGS's commit-then-recover design — seven independent experiments now confirm it's not salvageable by changing what gets revived. Plain (uncapped, mean-aggregation, fixed-width) beam search remains the production recommendation, and is now validated against both rule-level tweaks (§12-14) and a structurally different paradigm (§15) — nothing tried has come close to beating it. If going further: the main open angle left is improving the verifier signal itself (the gold-rank ceiling from §4 was never re-measured against beam search specifically).

## 17. State of the repo / housekeeping notes
